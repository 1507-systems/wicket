...
### `reload` subcommand (fixes `unlock` not refreshing rotated credentials)

- **Root cause**: a Tailscale OAuth client secret was rotated via `coffer set` while the daemon was running unlocked (its normal steady state — nothing routinely locks it). `wicket unlock` kept serving the stale value across two retry cycles. Traced to `Daemon.Unlock()`'s early-return guard (`if !d.locked.Load() { return nil }`): unlock only calls `LoadProviders()` — the thing that actually shells out to `coffer get` — when transitioning locked → unlocked. Called while already unlocked, it's a documented no-op (see `TestLockUnlockStateMachine`) that never re-reads coffer. Only `wicket stop && wicket start -d` worked, because `cmdStart` unconditionally calls `LoadProviders()`. The help text's claim that unlock "re-reads coffer" was true only for the locked→unlocked transition, not the common case.
- **Fix**: new `reload` protocol action + `wicket reload` CLI command. `Daemon.Reload()` always calls `LoadProviders()` regardless of lock state (erroring with `ErrReloadRequiresUnlock` / `LOCKED` if the daemon happens to be locked — reload deliberately does not implicitly unlock; `Lock` stays a real security boundary). No daemon restart, no socket drop, no other client's connection affected: `LoadProviders()` already built fresh provider instances into a new map and swapped it in atomically under `pmu`.
- **Hardening found along the way**: `LoadProviders()` was never closing the *outgoing* provider map on swap — harmless at startup (empty) and after `Lock()` (already closed), but reload is the first caller that replaces a live, in-use provider set with another live one, so the old (now-superseded, e.g. pre-rotation) credential would sit around unzeroed in an orphaned, unreferenced object rather than being explicitly cleared. Fixed by closing the previous map's providers (best-effort, logged not fatal) right after the swap; every provider's `Close()` is already idempotent so this is a safe no-op on the startup/unlock paths.
- Deliberately left the token cache alone on reload: a cached, still-live short-lived token remains valid regardless of whether the *root* credential that minted it was just rotated (rotating a meta-token doesn't revoke tokens already derived from it) — reload isn't `Lock`, so it doesn't force-evict.
- Tests: 55 passing (was ~43 as of the last log entry, plus PRs #4-#8's untested-in-log work) — 7 new: a real coffer round-trip via a fake `coffer` executable installed on `PATH` for the test (proves a rotated vault value is actually picked up, and that reload doesn't disturb providers whose value didn't change), a superseded-instance-is-closed check, locked-daemon behavior (both `Reload()` and `handleReload`), the wire response, and cache-survives-reload.
- Docs: `SPEC.md` gained `### Action: unlock` (previously undocumented) and `### Action: reload` sections, the CLI table, and the `action` field's value list (which had been missing `unlock` too). `wicket help` output updated.
- `gofmt`, `go vet`, `go build ./...`, `go test ./... -race` all clean.
- Deployed via `make install` (verify-or-rollback path already in the Makefile); live regression-tested against the real coffer vault using a low-risk round-tripped value, confirming `wicket reload` (not a restart) served the new value on the next `wicket get`. See Cortex `ext:feedback_wicket_unlock_does_not_refresh_cache` for the incident this closes out.
